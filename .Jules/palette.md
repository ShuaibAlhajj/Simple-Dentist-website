...
## 2026-05-24 - Interactive FAQ Accordion and Grid Transitions

**Learning:** Static FAQ lists can be enhanced with an interactive accordion pattern using Tailwind CSS's Grid `grid-rows-[0fr]` to `grid-rows-[1fr]` transition technique. This provides smooth height animations without needing to calculate pixel values in JavaScript. Accessibility is maintained by using button triggers with correct ARIA attributes.

**Action:** When implementing accordions, use the Tailwind Grid rows pattern for transitions and ensure an inner wrapper with `min-h-0` is present to allow for clean collapse. Use centralized JS logic to manage exclusive expansion (single-open) behavior.
