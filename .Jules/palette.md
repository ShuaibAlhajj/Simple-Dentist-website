...
## 2026-05-24 - Accessible Accordions and Robust Nav States

**Learning:** Implementing accessible accordions requires a combination of semantic HTML (buttons inside headings), ARIA attributes (`aria-expanded`, `aria-controls`), and smooth transitions (Tailwind Grid pattern). Furthermore, dynamic navigation highlighting must be defensive, explicitly clearing possible conflicting classes to ensure a consistent visual state across different page structures.

**Action:** Use the CSS Grid height transition pattern for accordions to handle dynamic content gracefully. Always sanitize element classes before applying active states in shared navigation logic to prevent styling collisions.
