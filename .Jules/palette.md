...
## 2026-05-24 - Accessible Accordion Pattern and Smooth Height Transitions

**Learning:** Interactive FAQ accordions improve information hierarchy by reducing visual clutter. A robust and accessible pattern involves using semantic headings for triggers, explicitly linking buttons to content panels with `aria-controls`, and implementing smooth height transitions using the Tailwind CSS Grid pattern (`grid-rows-[0fr]` to `grid-rows-[1fr]`) which avoids the limitations of fixed-height transitions.

**Action:** For interactive collapsible components, utilize the `data-faq-accordion` container pattern with a single-open JavaScript loop that manages both ARIA states and visual feedback (chevron rotation, borders, shadows) to ensure a consistent and delightful user experience.
