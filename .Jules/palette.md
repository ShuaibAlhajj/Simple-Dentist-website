...
## 2026-05-24 - W3C Accessible Smooth Accordions

**Learning:** Creating a highly accessible and visually delightful interactive accordion requires combining proper semantic structural markup (like wrapping toggle buttons in headers and using `aria-controls` with `role="region"`) and animating layout height smoothly without causing layout shifts. Utilizing Tailwind CSS CSS-Grid classes (`grid-rows-[0fr]` to `grid-rows-[1fr]`) over elements containing a `min-h-0` child is the most robust and clean way to achieve this.

**Action:** Use `role="region"`, `aria-controls`, and `aria-expanded` attributes to fully define the accessible states for expand/collapse elements. Always prefer CSS-Grid transition height animations for butter-smooth interactions.
